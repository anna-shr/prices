...
function Card(props) {
  const [selected, toggleSeletion] = useState(false);

  const handleHover = () => {
    toggleSeletion(!selected);
  };

  return (
    <div className={"card-wrapper " + (selected ? "selected" : "")} onMouseEnter={handleHover} onMouseLeave={handleHover}>
